...
def _verify_external_provenance(rule: dict, sca_source: str, rel_path) -> bool:
    """Verify the rule-generation extension provenance of a ``.sca`` rule (see
    CONSTRAINTS.md C9, its internal design docs §7.2).

    Only applies to rules carrying ``metadata.generated_by ==
    "external"`` — no effect on builtin or manually written custom rules
    (see ARCHITECTURE.md §7.3: "no overhead for builtin rules not
    generated by the rule-generation extension"). Returns False (rule to discard) with an
    ...
    if not signature:
        logger.warning(
            "Règle .sca ignorée (marqueur the rule-generation extension attendu mais signature absente) : %s",
            rel_path,
        )
        return False
    ...
    try:
        ...
        logger.warning(
            "Règle .sca ignorée (vérification de signature the rule-generation extension indisponible) : %s — %s",
            rel_path, exc,
        )
        return False
    ...
    if not verify_ed25519(content_hash, signature):
        logger.warning(
            "Règle .sca ignorée (signature the rule-generation extension invalide — fichier modifié manuellement ?) : %s",
            rel_path,
        )
        return False
...
def _load_sca_rules(sca_file: Path, rules_dir: Path, source: str) -> List[dict]:
    ...
    for rule in json_rules:
        ...
        for field in ("id", "language", "category", "severity", "mode", "message"):
            ...
            # the rule-generation extension provenance (C9) — only if the rule carries the
            # generated_by=external marker, no effect on other rules.
            meta = rule.get("metadata", {}) or {}
            if meta.get("generated_by") == "external":
                if not _verify_external_provenance(rule, sca_source, f"{rel_path}#{rule_id}"):
                    continue
